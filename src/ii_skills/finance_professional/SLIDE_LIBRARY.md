# Comprehensive Slide Library
## IC Presentations + Equity Research + Infrastructure IR

*Part 1: IC Presentation Templates (BCI Growth Equity, Northleaf PE Case Studies)*
*Part 2: Equity Research Templates (10 Years of CFA Research Challenge Winners, 2016-2025)*
*Location: `src/ii_skills/finance_professional/SLIDE_LIBRARY.md` — shared by IB Toolkit, IR Toolkit*

---

## Growth Equity / Venture IC Presentation

### Slide 1: Title Slide
```
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│                    [COMPANY LOGO]                           │
│                                                             │
│              SERIES [X] INVESTMENT OPPORTUNITY              │
│                                                             │
│                 Investment Committee Memo                    │
│                                                             │
│        ─────────────────────────────────────────           │
│                                                             │
│                    [FIRM NAME]                              │
│                    [Date]                                   │
│                                                             │
│         RECOMMENDATION: [BUY / QUALIFIED BUY / PASS]        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 2: Executive Summary
```
┌─────────────────────────────────────────────────────────────┐
│ EXECUTIVE SUMMARY                                           │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ COMPANY SNAPSHOT                                            │
│ ┌─────────────────┬─────────────────────────────────────┐  │
│ │ Sector          │ [e.g., Office of the CFO Software]  │  │
│ │ Founded         │ [Year]                              │  │
│ │ Headquarters    │ [City, State]                       │  │
│ │ Employees       │ [Number]                            │  │
│ │ Customers       │ [Number]                            │  │
│ └─────────────────┴─────────────────────────────────────┘  │
│                                                             │
│ TRANSACTION SUMMARY                                         │
│ • Investment: $[XX]M for [X.X]% ownership                  │
│ • Pre-Money: $[XXX]M ([X.X]x LTM Revenue)                  │
│ • Post-Money: $[XXX]M                                      │
│                                                             │
│ KEY METRICS                          RETURNS (BASE CASE)    │
│ • LTM Revenue: $[XX]M               • MOIC: [X.XX]x        │
│ • YoY Growth: [XX]%                 • IRR: [XX.X]%         │
│ • NDR: [XXX]%                       • Hold Period: [X] yrs │
│ • Gross Margin: [XX]%               • Exit Multiple: [X]x  │
│                                                             │
│ INVESTMENT THESIS                                           │
│ 1. [First pillar - one sentence]                           │
│ 2. [Second pillar - one sentence]                          │
│ 3. [Third pillar - one sentence]                           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 3: Company Overview
```
┌─────────────────────────────────────────────────────────────┐
│ COMPANY OVERVIEW: [COMPANY NAME]                            │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ FOUNDING STORY                                              │
│ [2-3 sentences on origin, founder background, problem]     │
│                                                             │
│ PRODUCT                                                     │
│ [Description of core product/service and differentiation]  │
│                                                             │
│ TRACTION                      FUNDING HISTORY               │
│ • [XX]K customers             • Seed: $[X]M ([Year])       │
│ • $[XX]M LTM revenue          • Series A: $[X]M ([Year])   │
│ • [XX]% MoM growth            • Series B: $[X]M ([Year])   │
│                               • Total Raised: $[XX]M       │
│                                                             │
│ KEY DIFFERENTIATORS                                         │
│ ┌─────────────┬─────────────┬─────────────┐                │
│ │ [Diff. 1]   │ [Diff. 2]   │ [Diff. 3]   │                │
│ │ [Detail]    │ [Detail]    │ [Detail]    │                │
│ └─────────────┴─────────────┴─────────────┘                │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 4: Investment Thesis
```
┌─────────────────────────────────────────────────────────────┐
│ INVESTMENT THESIS                                           │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ THESIS 1: [HEADLINE - Market/Competitive Advantage]        │
│ ─────────────────────────────────────────────────────────  │
│ • [Supporting data point 1]                                │
│ • [Supporting data point 2]                                │
│ • [Supporting data point 3]                                │
│ → Implication: [Why this drives returns]                   │
│                                                             │
│ THESIS 2: [HEADLINE - Distribution/Growth Engine]          │
│ ─────────────────────────────────────────────────────────  │
│ • [Supporting data point 1]                                │
│ • [Supporting data point 2]                                │
│ • [Supporting data point 3]                                │
│ → Implication: [Why this drives returns]                   │
│                                                             │
│ THESIS 3: [HEADLINE - Metrics Quality/Efficiency]          │
│ ─────────────────────────────────────────────────────────  │
│ • [Supporting data point 1]                                │
│ • [Supporting data point 2]                                │
│ • [Supporting data point 3]                                │
│ → Implication: [Why this drives returns]                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 5: Market Opportunity
```
┌─────────────────────────────────────────────────────────────┐
│ MARKET OPPORTUNITY                                          │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ MARKET SIZE                                                 │
│ ┌──────────────────────────────────────────────────────┐   │
│ │  TAM: $[XXX]B     SAM: $[XX]B      SOM: $[X]B       │   │
│ │  (Global)         (Region/Segment)  (Realistic)      │   │
│ └──────────────────────────────────────────────────────┘   │
│                                                             │
│ MARKET GROWTH                                               │
│ • [XX]% CAGR (20XX-20XX)                                   │
│ • Growing from $[XX]B to $[XX]B                            │
│                                                             │
│ WHY NOW?                                                    │
│ ┌─────────────────┬─────────────────┬─────────────────┐    │
│ │ TAILWIND 1      │ TAILWIND 2      │ TAILWIND 3      │    │
│ │ [Description]   │ [Description]   │ [Description]   │    │
│ └─────────────────┴─────────────────┴─────────────────┘    │
│                                                             │
│ HEADWINDS (Acknowledged)                                    │
│ • [Headwind 1 and why it's manageable]                     │
│ • [Headwind 2 and why it's manageable]                     │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 6: Competitive Landscape
```
┌─────────────────────────────────────────────────────────────┐
│ COMPETITIVE LANDSCAPE                                       │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ POSITIONING MATRIX                                          │
│                                                             │
│              [AXIS 2 LABEL]                                 │
│                    ▲                                        │
│       High         │                                        │
│                    │   ● [Comp 1]                          │
│    [Comp 4] ●      │         ● [Comp 2]                    │
│                    │                                        │
│                    │      ● [COMPANY]                       │
│       ─────────────┼─────────────────────►                 │
│                    │               [AXIS 1 LABEL]          │
│    [Comp 5] ●      │                                        │
│       Low          │                                        │
│                                                             │
│ KEY INSIGHT: [One sentence on competitive positioning]     │
│                                                             │
│ WIN/LOSS ANALYSIS                                           │
│ ┌────────────┬──────────┬─────────────┬─────────────┐      │
│ │ Competitor │ Win Rate │ Win Reasons │ Loss Reasons│      │
│ ├────────────┼──────────┼─────────────┼─────────────┤      │
│ │ [Comp A]   │ [XX]%    │ [Reasons]   │ [Reasons]   │      │
│ │ [Comp B]   │ [XX]%    │ [Reasons]   │ [Reasons]   │      │
│ │ [Comp C]   │ [XX]%    │ [Reasons]   │ [Reasons]   │      │
│ └────────────┴──────────┴─────────────┴─────────────┘      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 7: Unit Economics / GTM
```
┌─────────────────────────────────────────────────────────────┐
│ UNIT ECONOMICS & GO-TO-MARKET                               │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ CHANNEL ECONOMICS                                           │
│ ┌───────────────┬─────────┬─────────┬──────────┬─────────┐ │
│ │ Channel       │ CAC     │ LTV     │ LTV:CAC  │ Payback │ │
│ ├───────────────┼─────────┼─────────┼──────────┼─────────┤ │
│ │ PLG Direct    │ $[XXX]  │ $[X,XXX]│ [XX]x    │ [X] mo  │ │
│ │ Enterprise    │ $[X,XXX]│ $[XX,XXX]│ [X]x    │ [XX] mo │ │
│ │ Partner/Ref.  │ $[XX]   │ $[X,XXX]│ [XXX]x   │ [X] mo  │ │
│ └───────────────┴─────────┴─────────┴──────────┴─────────┘ │
│                                                             │
│ CURRENT CHANNEL MIX           TARGET MIX (20XX)            │
│ ┌─────────────────────┐       ┌─────────────────────┐      │
│ │ PLG:       [XX]%    │       │ PLG:       [XX]%    │      │
│ │ Enterprise:[XX]%    │  →    │ Enterprise:[XX]%    │      │
│ │ Partner:   [XX]%    │       │ Partner:   [XX]%    │      │
│ └─────────────────────┘       └─────────────────────┘      │
│                                                             │
│ KEY GTM INSIGHT:                                            │
│ [One sentence on why channel strategy drives growth]       │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 8: Financial Overview
```
┌─────────────────────────────────────────────────────────────┐
│ FINANCIAL OVERVIEW                                          │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ REVENUE TRAJECTORY — [CASE NAME] CASE                       │
│ ┌─────────┬────────┬────────┬────────┬────────┬────────┐   │
│ │ Metric  │ 2024A  │ 2025E  │ 2026E  │ 2027E  │ 5Y CAGR│   │
│ ├─────────┼────────┼────────┼────────┼────────┼────────┤   │
│ │ Revenue │ $[XX]M │ $[XX]M │ $[XXX]M│ $[XXX]M│ [XX]%  │   │
│ │ Growth  │ [XX]%  │ [XX]%  │ [XX]%  │ [XX]%  │ —      │   │
│ │ GM %    │ [XX]%  │ [XX]%  │ [XX]%  │ [XX]%  │ —      │   │
│ │ EBITDA %│ ([XX]%)│ ([XX]%)│ [X]%   │ [XX]%  │ —      │   │
│ └─────────┴────────┴────────┴────────┴────────┴────────┘   │
│                                                             │
│ PATH TO PROFITABILITY                                       │
│ • EBITDA breakeven: [Year] (Year [X] of investment)        │
│ • Key driver: [S&M leverage / Gross margin expansion]      │
│                                                             │
│ MODEL ASSUMPTIONS VS. MANAGEMENT                            │
│ ┌────────────────┬────────────┬────────────┬─────────────┐ │
│ │ Driver         │ Mgmt Case  │ [Firm] Case│ Delta       │ │
│ ├────────────────┼────────────┼────────────┼─────────────┤ │
│ │ NDR Trajectory │ [XXX]→[XXX]│ [XXX]→[XXX]│ ([XX]pp)    │ │
│ │ Growth Rate    │ [XX]%      │ [XX]%      │ ([XX]pp)    │ │
│ │ 20XX Revenue   │ $[XXX]M    │ $[XXX]M    │ ([XX]%)     │ │
│ └────────────────┴────────────┴────────────┴─────────────┘ │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slides 9-10: Returns Analysis
```
┌─────────────────────────────────────────────────────────────┐
│ RETURNS ANALYSIS                                            │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ BASE CASE PARAMETERS                                        │
│ • Entry: $[XXX]M pre-money ([X.X]x LTM Revenue)            │
│ • Exit: 20XX at [X.X]x LTM Revenue                         │
│ • Exit Revenue: $[XXX]M                                    │
│ • Exit TEV: $[X.X]B                                        │
│                                                             │
│ MOIC SENSITIVITY (Entry vs. Exit Multiple)                  │
│ ┌──────────────┬─────────┬─────────┬─────────┬─────────┐   │
│ │ Entry \ Exit │  5.0x   │  7.0x   │  8.0x   │ 10.0x   │   │
│ ├──────────────┼─────────┼─────────┼─────────┼─────────┤   │
│ │ $[XXX]M(10x) │ [X.XX]x │ [X.XX]x │ [X.XX]x │ [X.XX]x │   │
│ │ $[XXX]M(12x) │ [X.XX]x │[[X.XX]x]│ [X.XX]x │ [X.XX]x │   │
│ │ $[X,XXX]M(15x)│[X.XX]x │ [X.XX]x │ [X.XX]x │ [X.XX]x │   │
│ └──────────────┴─────────┴─────────┴─────────┴─────────┘   │
│                           [Base case highlighted]           │
│                                                             │
│ SCENARIO ANALYSIS                                           │
│ ┌──────────┬───────┬────────┬────────┬────────┬─────────┐  │
│ │ Scenario │ Prob. │ Exit   │ MOIC   │ IRR    │ Exit TEV│  │
│ ├──────────┼───────┼────────┼────────┼────────┼─────────┤  │
│ │ Bull     │ [XX]% │ [XX]x  │ [X.XX]x│ [XX]%  │ $[X.X]B │  │
│ │ Base     │ [XX]% │ [X]x   │ [X.XX]x│ [XX]%  │ $[X.X]B │  │
│ │ Bear     │ [XX]% │ [X]x   │ [X.XX]x│ [XX]%  │ $[X.X]B │  │
│ │ Downside │ [X]%  │ [X]x   │ [X.XX]x│ [X]%   │ $[XXX]M │  │
│ ├──────────┼───────┼────────┼────────┼────────┼─────────┤  │
│ │ Blended  │ 100%  │ —      │ [X.XX]x│ [XX]%  │ —       │  │
│ └──────────┴───────┴────────┴────────┴────────┴─────────┘  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 11: Risk Analysis
```
┌─────────────────────────────────────────────────────────────┐
│ RISK ANALYSIS                                               │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ KEY RISKS                                                   │
│ ┌────────────────┬───────┬──────────┬─────────────────────┐│
│ │ Risk           │ Prob. │ Severity │ Mitigation          ││
│ ├────────────────┼───────┼──────────┼─────────────────────┤│
│ │ [Risk 1]       │ [XX]% │ HIGH     │ [Specific action]   ││
│ │ [Risk 2]       │ [XX]% │ HIGH     │ [Specific action]   ││
│ │ [Risk 3]       │ [XX]% │ MED-HIGH │ [Specific action]   ││
│ │ [Risk 4]       │ [XX]% │ MEDIUM   │ [Specific action]   ││
│ └────────────────┴───────┴──────────┴─────────────────────┘│
│                                                             │
│ DOWNSIDE SCENARIO                                           │
│ • Trigger: [What causes bear case]                         │
│ • Impact: Growth slows to [XX]%, exit at [X]x              │
│ • Returns: [X.XX]x MOIC / [XX]% IRR                        │
│ • Assessment: [Still recover capital / Loss scenario]      │
│                                                             │
│ DOWNSIDE PROTECTION                                         │
│ • [Liquidation preference / Board rights / etc.]           │
│ • Break-even multiple: [X.X]x (vs. market median [X.X]x)   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 12: "What Must Be True"
```
┌─────────────────────────────────────────────────────────────┐
│ WHAT MUST BE TRUE                                           │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ For base case returns to materialize:                       │
│                                                             │
│ 1. GROWTH                                                   │
│    □ [Specific threshold, e.g., ">40% YoY through 2028"]   │
│    If not: [Impact on returns]                             │
│                                                             │
│ 2. MARKET                                                   │
│    □ [External condition, e.g., "No QB AI before H2 2027"] │
│    If not: [Impact on returns]                             │
│                                                             │
│ 3. RETENTION                                                │
│    □ [Metric threshold, e.g., "NDR ≥120% sustained"]       │
│    If not: [Impact on returns]                             │
│                                                             │
│ 4. EXIT                                                     │
│    □ [Market condition, e.g., "Multiples ≥7.0x LTM"]       │
│    If not: [Impact on returns]                             │
│                                                             │
│ ─────────────────────────────────────────────────────────  │
│ If ALL conditions hold → Base case: [X.XX]x MOIC           │
│ If market improves    → Bull case: [X.XX]x MOIC            │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Slide 13: Recommendation
```
┌─────────────────────────────────────────────────────────────┐
│ RECOMMENDATION                                              │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│              ┌─────────────────────────────┐                │
│              │    [QUALIFIED] BUY          │                │
│              └─────────────────────────────┘                │
│                                                             │
│ TRANSACTION TERMS                                           │
│ • Investment: $[XX]M ([lead/co-invest])                    │
│ • Valuation: $[XXX]M pre / $[XXX]M post                    │
│ • Ownership: [X.X]%                                        │
│ • Board Seat: [Yes/No] with [specific rights]              │
│ • Key Conditions: [CFO hire, approval rights, etc.]        │
│                                                             │
│ EXPECTED RETURNS                                            │
│ ┌──────────────┬─────────────┬─────────────┐               │
│ │ Bull Case    │ Base Case   │ Bear Case   │               │
│ │ [X.XX]x MOIC │ [X.XX]x MOIC│ [X.XX]x MOIC│               │
│ │ [XX]% IRR    │ [XX]% IRR   │ [XX]% IRR   │               │
│ └──────────────┴─────────────┴─────────────┘               │
│                                                             │
│ WHY WE'RE CONFIDENT                                         │
│ 1. [Reason 1 - validated thesis pillar]                    │
│ 2. [Reason 2 - validated thesis pillar]                    │
│ 3. [Reason 3 - conservative underwriting]                  │
│                                                             │
│ DILIGENCE GATES BEFORE CLOSE                                │
│ 1. [Gate 1 with pass/fail criteria]                        │
│ 2. [Gate 2 with pass/fail criteria]                        │
│ 3. [Gate 3 with pass/fail criteria]                        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## LBO / PE Investment Presentation

### Additional Slides for LBO Decks

### Sources & Uses Slide
```
┌─────────────────────────────────────────────────────────────┐
│ SOURCES & USES                                              │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ SOURCES                       USES                          │
│ ┌─────────────────────────┐   ┌─────────────────────────┐  │
│ │ Senior Debt    $[XXX]M  │   │ Purchase Price $[XXX]M  │  │
│ │ ([X.X]x EBITDA)         │   │ ([X.X]x EBITDA)         │  │
│ │                         │   │                         │  │
│ │ Mezzanine      $[XX]M   │   │ Refinance Debt $[XX]M   │  │
│ │ ([X.X]x EBITDA)         │   │                         │  │
│ │                         │   │ Transaction   $[XX]M    │  │
│ │ Equity         $[XXX]M  │   │ Fees                    │  │
│ │ ([XX]% of TEV)          │   │                         │  │
│ │                         │   │ Cash to B/S   $[X]M     │  │
│ ├─────────────────────────┤   ├─────────────────────────┤  │
│ │ TOTAL          $[XXX]M  │   │ TOTAL         $[XXX]M   │  │
│ └─────────────────────────┘   └─────────────────────────┘  │
│                                                             │
│ KEY CREDIT METRICS                                          │
│ • Total Leverage: [X.X]x EBITDA                            │
│ • Senior Leverage: [X.X]x EBITDA                           │
│ • Interest Coverage: [X.X]x                                │
│ • Fixed Charge Coverage: [X.X]x                            │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Value Creation Bridge
```
┌─────────────────────────────────────────────────────────────┐
│ VALUE CREATION BRIDGE                                       │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Entry Equity    $[XXX]M ████████████                      │
│                          │                                  │
│  + EBITDA Growth $[XX]M  ████████████████                  │
│    ([XX]% CAGR)          │                                  │
│                          │                                  │
│  + Multiple Exp. $[XX]M  ████████████████████              │
│    ([X.X]x → [X.X]x)     │                                  │
│                          │                                  │
│  + Debt Paydown  $[XX]M  ████████████████████████          │
│    ($[XX]M principal)    │                                  │
│                          │                                  │
│  - Txn Costs    ($[X]M)  ████████████████████████          │
│                          │                                  │
│  = Exit Equity   $[XXX]M ████████████████████████████████  │
│                                                             │
│  MOIC: [X.X]x    IRR: [XX]%                                │
│                                                             │
│ VALUE CREATION ATTRIBUTION                                  │
│ • EBITDA Growth: [XX]% of value creation                   │
│ • Multiple Expansion: [XX]% of value creation              │
│ • Deleveraging: [XX]% of value creation                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Football Field (Valuation Summary)
```
┌─────────────────────────────────────────────────────────────┐
│ VALUATION SUMMARY — FOOTBALL FIELD                          │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│                   $[XX]M        $[XX]M        $[XX]M        │
│                     │             │             │           │
│ DCF               ████████████████████████████████████     │
│ (WACC: [X]%)        │←──────────────────────────→│         │
│                     │             │             │           │
│ Trading Comps     ████████████████████████████             │
│ ([X-X]x EBITDA)     │←────────────────────→│               │
│                     │             │             │           │
│ Precedent Txns              ████████████████████████████   │
│ ([X-X]x EBITDA)               │←────────────────────→│     │
│                     │             │             │           │
│ LBO Analysis      ████████████████████████                 │
│ ([XX]% IRR floor)   │←────────────────→│                   │
│                     │             │             │           │
│                   $[XX]M        $[XX]M        $[XX]M        │
│                                   │                         │
│                              Implied TEV                    │
│                                                             │
│ VALUATION RANGE: $[XX]M - $[XX]M                           │
│ MIDPOINT: $[XX]M ([X.X]x EBITDA)                           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## LBO / PE Additional Slides (Continued)

### SaaS Peer Benchmarking with Quartiles
```
┌─────────────────────────────────────────────────────────────┐
│ PEER BENCHMARKING ANALYSIS                        [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ ┌──────────────────┬────────┬────────┬────────┬────────────┐│
│ │ Metric           │ Bottom │  2nd   │  3rd   │   Top      ││
│ │                  │Quartile│Quartile│Quartile│ Quartile   ││
│ │                  │ (Red)  │(Yellow)│(Lt Grn)│  (Green)   ││
│ ├──────────────────┼────────┼────────┼────────┼────────────┤│
│ │ GROWTH           │        │        │        │            ││
│ │ ARR Growth (YoY) │  <9%   │  9-13% │ 13-21% │   >21%     ││
│ │ [Company: 35%]   │        │        │        │    ████    ││
│ ├──────────────────┼────────┼────────┼────────┼────────────┤│
│ │ RETENTION        │        │        │        │            ││
│ │ Net Dollar Ret.  │  <97%  │97-105% │105-115%│  >115%     ││
│ │ [Company: 118%]  │        │        │        │    ████    ││
│ │ Gross Dollar Ret.│  <78%  │ 78-85% │ 85-90% │   >90%     ││
│ │ [Company: 92%]   │        │        │        │    ████    ││
│ ├──────────────────┼────────┼────────┼────────┼────────────┤│
│ │ EFFICIENCY       │        │        │        │            ││
│ │ Magic Number     │ <0.37x │0.37-0.47│0.47-0.72│ >0.72x    ││
│ │ [Company: 0.85x] │        │        │        │    ████    ││
│ │ LTV/CAC          │ <2.0x  │2.0-3.0x│3.0-4.5x│  >4.5x    ││
│ │ [Company: 3.8x]  │        │        │ ████   │            ││
│ │ Burn Multiple    │ >3.5x  │2.5-3.5x│1.8-2.5x│  <1.8x    ││
│ │ [Company: 1.5x]  │        │        │        │    ████    ││
│ └──────────────────┴────────┴────────┴────────┴────────────┘│
│                                                             │
│ Source: Drake Star, KeyBanc SaaS Survey, Company data       │
└─────────────────────────────────────────────────────────────┘
```

**Key Rules:**
- Quartile thresholds from industry surveys (Drake Star, KeyBanc)
- Color-coded columns: Red → Yellow → Light Green → Green
- Company's actual metric shown with quartile position highlighted
- Group by category (Growth, Retention, Efficiency, Unit Economics)

**Pattern:** BCI Growth Equity Case Study (SaaS Benchmarking)

---

### Precedent Transactions Table
```
┌─────────────────────────────────────────────────────────────┐
│ PRECEDENT TRANSACTIONS                            [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ ┌───────┬──────┬───────┬───────┬──────┬──────┬────────────┐│
│ │ Date  │Target│Acquir.│EV ($M)│EV/Rev│EV/   │ Rev Growth ││
│ │       │      │       │       │      │EBITDA│            ││
│ ├───────┼──────┼───────┼───────┼──────┼──────┼────────────┤│
│ │ Q1'25 │ Co A │ PE A  │ $2,500│ 8.5x │ 22.0x│    28%     ││
│ │ Q4'24 │ Co B │ PE B  │ $1,800│ 7.2x │ 19.5x│    35%     ││
│ │ Q3'24 │ Co C │ Strat │ $3,200│10.1x │ 25.0x│    42%     ││
│ │ Q2'24 │ Co D │ PE C  │ $1,200│ 6.8x │ 18.0x│    22%     ││
│ │ Q1'24 │ Co E │ PE D  │ $4,500│12.0x │ 28.0x│    55%     ││
│ ├───────┼──────┼───────┼───────┼──────┼──────┼────────────┤│
│ │       │      │ Mean  │       │ 8.9x │ 22.5x│    36%     ││
│ │       │      │Median │       │ 8.5x │ 22.0x│    35%     ││
│ ├───────┼──────┼───────┼───────┼──────┼──────┼────────────┤│
│ │       │[SUBJ]│ Impl. │ $[XX] │[X.X]x│[XX]x │   [XX]%    ││ ← Green
│ └───────┴──────┴───────┴───────┴──────┴──────┴────────────┘│
│                                                             │
│ Source: Capital IQ, PitchBook, Company filings              │
└─────────────────────────────────────────────────────────────┘
```

**Key Rules:**
- Sorted by date (most recent first)
- Subject company row highlighted green at bottom
- Mean/Median summary row
- Include deal context (strategic vs. PE buyer, growth rate)

---

### DCF Valuation Summary
```
┌─────────────────────────────────────────────────────────────┐
│ DCF VALUATION                                     [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ KEY ASSUMPTIONS                                             │
│ ┌─────────────────────────────────────────────────────────┐ │
│ │ WACC: [XX.X]%  │  TGR: [X.X]%  │  Projection: [X] yrs │ │
│ └─────────────────────────────────────────────────────────┘ │
│                                                             │
│ FREE CASH FLOW PROJECTION                                   │
│ ┌────────┬────────┬────────┬────────┬────────┬────────────┐ │
│ │        │ 2025E  │ 2026E  │ 2027E  │ 2028E  │ 2029E      │ │
│ ├────────┼────────┼────────┼────────┼────────┼────────────┤ │
│ │ Revenue│ $[XX]M │ $[XX]M │ $[XXX]M│ $[XXX]M│ $[XXX]M    │ │
│ │ EBITDA │ $[XX]M │ $[XX]M │ $[XX]M │ $[XX]M │ $[XX]M     │ │
│ │ uFCF   │ $[XX]M │ $[XX]M │ $[XX]M │ $[XX]M │ $[XX]M     │ │
│ └────────┴────────┴────────┴────────┴────────┴────────────┘ │
│                                                             │
│ VALUATION BUILD                                             │
│ ┌─────────────────────────────────────────────────────────┐ │
│ │ PV of Projection Period FCF              $[XXX]M        │ │
│ │ + PV of Terminal Value                   $[XXX]M        │ │
│ │ = Enterprise Value                       $[XXX]M        │ │
│ │ - Net Debt                               ($[XX]M)       │ │
│ │ = Equity Value                           $[XXX]M        │ │
│ │                                                         │ │
│ │ Implied EV/EBITDA: [XX.X]x                              │ │
│ │ Implied EV/Revenue: [X.X]x                              │ │
│ └─────────────────────────────────────────────────────────┘ │
│                                                             │
│ SENSITIVITY (WACC vs. TGR)                                  │
│ ┌──────────┬────────┬────────┬────────┬────────┐            │
│ │ WACC\TGR │  1.5%  │  2.0%  │  2.5%  │  3.0%  │            │
│ ├──────────┼────────┼────────┼────────┼────────┤            │
│ │   9.0%   │ $[XXX] │ $[XXX] │ $[XXX] │ $[XXX] │            │
│ │  10.0%   │ $[XXX] │[$[XXX]]│ $[XXX] │ $[XXX] │            │
│ │  11.0%   │ $[XXX] │ $[XXX] │ $[XXX] │ $[XXX] │            │
│ └──────────┴────────┴────────┴────────┴────────┘            │
│                           [Base case highlighted]           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

### Sensitivity Matrix (Entry vs. Exit Multiple)
```
┌─────────────────────────────────────────────────────────────┐
│ RETURN SENSITIVITY                                [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ MOIC SENSITIVITY (Entry Multiple vs. Exit Multiple)         │
│ ┌───────────────┬────────┬────────┬────────┬────────┐       │
│ │ Entry \ Exit  │  6.0x  │  8.0x  │ 10.0x  │ 12.0x  │       │
│ ├───────────────┼────────┼────────┼────────┼────────┤       │
│ │ 6.0x ($117M)  │  2.4x  │  3.1x  │  3.9x  │  4.7x  │       │
│ │ 8.0x ($156M)  │  1.8x  │ [2.4x] │  2.9x  │  3.5x  │       │
│ │ 10.0x ($195M) │  1.4x  │  1.9x  │  2.4x  │  2.8x  │       │
│ │ 12.0x ($234M) │  1.2x  │  1.6x  │  2.0x  │  2.3x  │       │
│ └───────────────┴────────┴────────┴────────┴────────┘       │
│                                                             │
│ IRR SENSITIVITY (Entry Multiple vs. Exit Multiple)          │
│ ┌───────────────┬────────┬────────┬────────┬────────┐       │
│ │ Entry \ Exit  │  6.0x  │  8.0x  │ 10.0x  │ 12.0x  │       │
│ ├───────────────┼────────┼────────┼────────┼────────┤       │
│ │ 6.0x ($117M)  │  19.2% │  25.4% │  31.1% │  36.0% │       │
│ │ 8.0x ($156M)  │  12.3% │ [19.0%]│  24.5% │  29.2% │       │
│ │ 10.0x ($195M) │   7.2% │  13.8% │  19.5% │  24.0% │       │
│ │ 12.0x ($234M) │   3.4% │   9.8% │  15.1% │  19.7% │       │
│ └───────────────┴────────┴────────┴────────┴────────┘       │
│                                                             │
│ [X.Xx] = Base case highlighted   Green: >20% IRR           │
│ Source: Company model                                       │
└─────────────────────────────────────────────────────────────┘
```

**Key Rules:**
- Two matrices: MOIC and IRR
- Base case cell highlighted (blue border)
- Color gradient: Green (>20% IRR) → Yellow (10-20%) → Red (<10%)
- Entry equity shown alongside multiple

---

### IRR Attribution Bridge (Waterfall)
```
┌─────────────────────────────────────────────────────────────┐
│ KEY RETURN DRIVERS | BASE CASE                    [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ MOIC VALUE BRIDGE ($M)                                      │
│                                                             │
│              +$173                                          │
│           ┌────────┐                                        │
│   $97     │ Revenue│                              $290      │
│ ┌──────┐  │ Growth │  -$45  +$65                ┌────────┐  │
│ │Entry │  │        │┌──────┐┌────┐              │ Exit   │  │
│ │Equity│  │        ││Multi ││Debt│              │ Equity │  │
│ │      │  │ (Green)││Contr.││Paydn│              │(Navy)  │  │
│ │      │  │        ││(Red) ││(Grn)│              │        │  │
│ └──────┘  └────────┘└──────┘└────┘              └────────┘  │
│                                                             │
│ Implied MOIC: 3.0x                                          │
│                                                             │
│ IRR ATTRIBUTION BRIDGE (%)                                  │
│                                                             │
│              +35%                                           │
│           ┌────────┐                                        │
│    0%     │ Revenue│                                20%     │
│ ┌──────┐  │ Growth │  -14%   -1%                ┌────────┐  │
│ │Base  │  │        │┌──────┐┌────┐              │ Net    │  │
│ │(0%)  │  │        ││Multi ││Dil.│              │ IRR    │  │
│ │      │  │ (Green)││Contr.││(Red)│              │(Navy)  │  │
│ └──────┘  └────────┘└──────┘└────┘              └────────┘  │
│                                                             │
│ Returns primarily driven by revenue growth rather than      │
│ multiple expansion; assumed contraction protects downside    │
│                                                             │
│ Source: Company model                                       │
└─────────────────────────────────────────────────────────────┘
```

**Key Rules:**
- Stacked waterfall: invisible base + green increases + red decreases + navy total
- Both MOIC and IRR bridges on same slide
- Positive contributors in green, negative in red
- Entry/exit bars in navy
- Key insight text at bottom

**Pattern:** BCI 2026 Case Study Slide 18

---

### Debt Schedule & Credit Metrics
```
┌─────────────────────────────────────────────────────────────┐
│ DEBT SCHEDULE & CREDIT ANALYSIS                   [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ DEBT STRUCTURE                                              │
│ ┌──────────────┬────────┬──────┬──────┬──────┬────────────┐ │
│ │ Tranche      │ Amount │ Rate │ Mult.│Matur.│ Amort.     │ │
│ ├──────────────┼────────┼──────┼──────┼──────┼────────────┤ │
│ │ Revolver     │  $25M  │S+250 │ 0.0x │ 2030 │   —        │ │
│ │ Term Loan A  │ $100M  │S+300 │ 2.6x │ 2030 │ 5% / yr    │ │
│ │ Term Loan B  │  $75M  │S+400 │ 4.5x │ 2031 │ 1% / yr    │ │
│ │ Sr. Notes    │  $50M  │ 8.5% │ 5.8x │ 2032 │ Bullet     │ │
│ ├──────────────┼────────┼──────┼──────┼──────┼────────────┤ │
│ │ Total Debt   │ $250M  │      │ 5.8x │      │            │ │
│ │ (−) Cash     │ ($10M) │      │      │      │            │ │
│ │ Net Debt     │ $240M  │      │ 5.6x │      │            │ │
│ └──────────────┴────────┴──────┴──────┴──────┴────────────┘ │
│                                                             │
│ CREDIT METRICS OVER HOLD                                    │
│ ┌──────────┬────────┬────────┬────────┬────────┬──────────┐ │
│ │          │ Entry  │ Yr 1   │ Yr 2   │ Yr 3   │  Exit    │ │
│ ├──────────┼────────┼────────┼────────┼────────┼──────────┤ │
│ │Total Lev.│  5.8x  │  4.9x  │  4.1x  │  3.5x  │   2.8x   │ │
│ │Sr. Lev.  │  4.5x  │  3.8x  │  3.2x  │  2.7x  │   2.2x   │ │
│ │Int. Cov. │  2.1x  │  2.5x  │  2.9x  │  3.4x  │   4.0x   │ │
│ │FCCR      │  1.4x  │  1.7x  │  2.0x  │  2.3x  │   2.7x   │ │
│ └──────────┴────────┴────────┴────────┴────────┴──────────┘ │
│                                                             │
│ COVENANT HEADROOM                                           │
│ Max Leverage: 6.5x │ Current: 5.8x │ Cushion: 0.7x        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**Key Rules:**
- Full debt structure with tranches, rates, maturities
- Credit metrics projected over hold period
- Leverage shown declining (deleveraging story)
- Covenant headroom quantified

---

### Management vs. Buyer Case Comparison
```
┌─────────────────────────────────────────────────────────────┐
│ MANAGEMENT CASE vs. BUYER CASE                    [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ KEY ASSUMPTION DIFFERENCES                                  │
│ ┌────────────────┬──────────────┬──────────────┬──────────┐ │
│ │ Assumption     │  Mgmt Case   │  Buyer Case  │  Delta   │ │
│ ├────────────────┼──────────────┼──────────────┼──────────┤ │
│ │ Rev Growth '26 │    35%       │    28%       │  (7pp)   │ │
│ │ Rev Growth '27 │    30%       │    22%       │  (8pp)   │ │
│ │ NDR Trajectory │  125%→130%   │  118%→120%   │  (7-10pp)│ │
│ │ EBITDA Margin  │  -5%→+15%   │  -8%→+10%    │  (3-5pp) │ │
│ │ Exit Multiple  │   12.0x      │    8.0x      │  (4.0x)  │ │
│ ├────────────────┼──────────────┼──────────────┼──────────┤ │
│ │ Exit Revenue   │   $350M      │    $250M     │  (29%)   │ │
│ │ Exit EBITDA    │    $53M      │     $25M     │  (53%)   │ │
│ └────────────────┴──────────────┴──────────────┴──────────┘ │
│                                                             │
│ RETURNS COMPARISON                                          │
│ ┌────────────────┬──────────────┬──────────────┐            │
│ │                │  Mgmt Case   │  Buyer Case  │            │
│ ├────────────────┼──────────────┼──────────────┤            │
│ │ MOIC           │    5.2x      │    2.4x      │            │
│ │ IRR            │    38%       │    19%        │            │
│ │ Hold Period    │    5 yrs     │    5 yrs      │            │
│ └────────────────┴──────────────┴──────────────┘            │
│                                                             │
│ Conservative underwriting: Buyer case assumptions used      │
│ for base case returns; mgmt case provides upside optionality│
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

### Operating Model / P&L Summary
```
┌─────────────────────────────────────────────────────────────┐
│ OPERATING MODEL                                   [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ ┌──────────┬────────┬────────┬────────┬────────┬──────────┐ │
│ │ ($M)     │ 2024A  │ 2025E  │ 2026E  │ 2027E  │ 2028E    │ │
│ ├──────────┼────────┼────────┼────────┼────────┼──────────┤ │
│ │ Revenue  │  $65.0 │  $87.8 │ $110.2 │ $134.4 │  $159.1  │ │
│ │  Growth  │   42%  │   35%  │   26%  │   22%  │    18%   │ │
│ ├──────────┼────────┼────────┼────────┼────────┼──────────┤ │
│ │ COGS     │ ($22.8)│ ($28.5)│ ($34.2)│ ($40.3)│ ($46.1)  │ │
│ │ Gross Pft│  $42.3 │  $59.3 │  $76.0 │  $94.1 │ $113.0   │ │
│ │  Margin  │   65%  │   68%  │   69%  │   70%  │    71%   │ │
│ ├──────────┼────────┼────────┼────────┼────────┼──────────┤ │
│ │ S&M      │ ($32.5)│ ($39.5)│ ($44.1)│ ($47.1)│ ($49.3)  │ │
│ │  % Rev   │   50%  │   45%  │   40%  │   35%  │    31%   │ │
│ │ R&D      │ ($16.3)│ ($19.3)│ ($22.0)│ ($24.2)│ ($25.5)  │ │
│ │  % Rev   │   25%  │   22%  │   20%  │   18%  │    16%   │ │
│ │ G&A      │  ($6.5)│  ($7.9)│  ($8.8)│  ($9.4)│  ($9.5)  │ │
│ │  % Rev   │   10%  │    9%  │    8%  │    7%  │     6%   │ │
│ ├──────────┼────────┼────────┼────────┼────────┼──────────┤ │
│ │ EBITDA   │ ($13.0)│  ($7.4)│   $1.1 │  $13.4 │  $28.7   │ │
│ │  Margin  │  (20%) │   (8%) │    1%  │   10%  │    18%   │ │
│ └──────────┴────────┴────────┴────────┴────────┴──────────┘ │
│                                                             │
│ PATH TO PROFITABILITY: EBITDA breakeven in 2026E (Year 2)   │
│ Key driver: S&M leverage (50% → 31% of revenue)            │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**Key Rules:**
- Revenue at top, EBITDA at bottom
- Growth rates and margins shown as percentages
- Negative values in parentheses
- Key driver highlighted at bottom

---

### Add-on / Synergy Analysis
```
┌─────────────────────────────────────────────────────────────┐
│ ADD-ON ACQUISITION ANALYSIS                       [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ TARGET PROFILE                                              │
│ ┌─────────────────┬─────────────────────────────────────┐   │
│ │ Revenue         │ $25M                                │   │
│ │ EBITDA          │ $5M (20% margin)                    │   │
│ │ Entry Multiple  │ 6.0x EBITDA                         │   │
│ │ Purchase Price  │ $30M                                │   │
│ └─────────────────┴─────────────────────────────────────┘   │
│                                                             │
│ SYNERGY BREAKDOWN                                           │
│ ┌──────────────────┬──────────┬──────────┬────────────────┐ │
│ │ Category         │ Year 1   │ Year 2   │ Full Run-Rate  │ │
│ ├──────────────────┼──────────┼──────────┼────────────────┤ │
│ │ Revenue Synergies│   $2M    │   $5M    │     $8M        │ │
│ │ Cost Synergies   │   $3M    │   $4M    │     $5M        │ │
│ │ Total Synergies  │   $5M    │   $9M    │    $13M        │ │
│ ├──────────────────┼──────────┼──────────┼────────────────┤ │
│ │ Integration Costs│  ($3M)   │  ($1M)   │      —         │ │
│ │ Net Benefit      │   $2M    │   $8M    │    $13M        │ │
│ └──────────────────┴──────────┴──────────┴────────────────┘ │
│                                                             │
│ PRO FORMA IMPACT                                            │
│ ┌────────────────┬───────────┬───────────┬────────────────┐ │
│ │                │Standalone │ + Add-on  │  Pro Forma     │ │
│ ├────────────────┼───────────┼───────────┼────────────────┤ │
│ │ Revenue        │  $150M    │   $25M    │    $175M       │ │
│ │ EBITDA         │   $30M    │   $5M     │     $35M       │ │
│ │ + Synergies    │     —     │     —     │    +$13M       │ │
│ │ PF EBITDA      │     —     │     —     │     $48M       │ │
│ │ PF Multiple    │     —     │     —     │     5.2x       │ │
│ └────────────────┴───────────┴───────────┴────────────────┘ │
│                                                             │
│ Acquisition multiple of 6.0x declines to 2.3x with synergies│
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

### Cap Table / Sponsor Economics
```
┌─────────────────────────────────────────────────────────────┐
│ SPONSOR ECONOMICS & CAP TABLE                     [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ CAPITALIZATION AT ENTRY                                     │
│ ┌──────────────────┬────────────┬──────────────────────────┐│
│ │                  │ Amount ($M)│ % of Total               ││
│ ├──────────────────┼────────────┼──────────────────────────┤│
│ │ Sponsor Equity   │    $85     │   87%                    ││
│ │ Mgmt Rollover    │     $8     │    8%                    ││
│ │ Mgmt Option Pool │     $5     │    5%  (vesting: 4 yrs) ││
│ ├──────────────────┼────────────┼──────────────────────────┤│
│ │ Total Equity     │    $98     │  100%                    ││
│ └──────────────────┴────────────┴──────────────────────────┘│
│                                                             │
│ RETURNS WATERFALL (Base Case: 3.0x MOIC)                    │
│ ┌──────────────────┬────────────┬─────────┬───────────────┐ │
│ │ Tier             │ Hurdle     │ Carry   │ Distribution  │ │
│ ├──────────────────┼────────────┼─────────┼───────────────┤ │
│ │ Return of Capital│ 1.0x       │   0%    │     $98M      │ │
│ │ Preferred Return │ 8% IRR     │   0%    │     $39M      │ │
│ │ GP Catch-up      │ 1.0x→1.25x │  100%   │     $25M      │ │
│ │ Carried Interest │ >1.25x     │  20%    │     $24M      │ │
│ ├──────────────────┼────────────┼─────────┼───────────────┤ │
│ │ Total Proceeds   │            │         │    $294M      │ │
│ │ LP Net Return    │            │         │    $245M (2.5x)│ │
│ │ GP Carry         │            │         │     $49M      │ │
│ └──────────────────┴────────────┴─────────┴───────────────┘ │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

### 100-Day Plan / Post-Close
```
┌─────────────────────────────────────────────────────────────┐
│ 100-DAY VALUE CREATION PLAN                       [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌──────────────────────────────────────────────────────┐   │
│  │ Day 1-30           │ Day 31-60         │ Day 61-100  │   │
│  │ STABILIZE           │ OPTIMIZE          │ ACCELERATE  │   │
│  ├──────────────────────┼───────────────────┼─────────────┤   │
│  │                      │                   │             │   │
│  │ □ Onboard new CFO   │ □ Implement new   │ □ Launch    │   │
│  │ □ Financial deep-   │   pricing model   │   add-on    │   │
│  │   dive & true-up    │ □ Renegotiate     │   pipeline  │   │
│  │ □ Customer retention│   vendor contracts│ □ New market│   │
│  │   audit             │ □ IT integration  │   expansion │   │
│  │ □ Management        │   roadmap         │ □ KPI       │   │
│  │   assessment        │ □ Quick-win cost  │   dashboard │   │
│  │ □ Board composition │   reductions      │   live      │   │
│  │                      │                   │             │   │
│  ├──────────────────────┼───────────────────┼─────────────┤   │
│  │ EBITDA Impact: —     │ +$2-3M run-rate  │ +$5-8M      │   │
│  └──────────────────────┴───────────────────┴─────────────┘   │
│                                                             │
│ KEY HIRES REQUIRED                                          │
│ • CFO (Month 1) • VP Sales (Month 2) • Head of CorpDev (3) │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**Key Rules:**
- Three time phases as columns
- Checklist format for action items
- EBITDA impact estimated per phase
- Key hires listed below

---

### Covenant Analysis
```
┌─────────────────────────────────────────────────────────────┐
│ COVENANT ANALYSIS                                 [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ FINANCIAL COVENANTS                                         │
│ ┌──────────────────┬──────────┬──────────┬────────────────┐ │
│ │ Covenant         │ Limit    │ Actual   │ Headroom       │ │
│ ├──────────────────┼──────────┼──────────┼────────────────┤ │
│ │ Max Total Lev.   │  ≤6.5x   │  5.8x    │ ✓ 0.7x cushion│ │
│ │ Max Senior Lev.  │  ≤5.0x   │  4.5x    │ ✓ 0.5x cushion│ │
│ │ Min Int. Coverage│  ≥1.5x   │  2.1x    │ ✓ 0.6x above  │ │
│ │ Min FCCR         │  ≥1.0x   │  1.4x    │ ✓ 0.4x above  │ │
│ │ Max Capex        │  $15M/yr │  $12M    │ ✓ $3M below   │ │
│ └──────────────────┴──────────┴──────────┴────────────────┘ │
│                                                             │
│ COVENANT STRESS TEST                                        │
│ ┌──────────────────┬──────────┬──────────┬────────────────┐ │
│ │ Scenario         │ Lev.     │ Int.Cov. │ Breach?        │ │
│ ├──────────────────┼──────────┼──────────┼────────────────┤ │
│ │ Base Case        │  5.8x    │  2.1x    │ No             │ │
│ │ -10% EBITDA      │  6.4x    │  1.9x    │ No             │ │
│ │ -20% EBITDA      │  7.2x    │  1.7x    │ YES (Lev.)     │ │
│ │ -30% EBITDA      │  8.3x    │  1.5x    │ YES (Lev.)     │ │
│ └──────────────────┴──────────┴──────────┴────────────────┘ │
│                                                             │
│ EBITDA can decline ~12% before first covenant is triggered  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

### Quality of Earnings Summary
```
┌─────────────────────────────────────────────────────────────┐
│ QUALITY OF EARNINGS ANALYSIS                      [LOGO]    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│ EBITDA BRIDGE: REPORTED → ADJUSTED                          │
│ ┌────────────────────────────────────────────┬────────────┐ │
│ │                                            │   Amount   │ │
│ ├────────────────────────────────────────────┼────────────┤ │
│ │ Reported EBITDA                            │   $42.0M   │ │
│ ├────────────────────────────────────────────┼────────────┤ │
│ │ Normalizing Adjustments:                   │            │ │
│ │  (+) Non-recurring legal costs             │   +$2.1M   │ │
│ │  (+) One-time restructuring                │   +$1.5M   │ │
│ │  (+) Below-market lease adjustment         │   +$0.8M   │ │
│ │  (−) Above-market revenue (one-time)       │   −$1.2M   │ │
│ │  (−) Aggressive capitalization             │   −$0.9M   │ │
│ │                                            │            │ │
│ │  Subtotal Adjustments                      │   +$2.3M   │ │
│ ├────────────────────────────────────────────┼────────────┤ │
│ │ Adjusted EBITDA                            │   $44.3M   │ │
│ │                                            │            │ │
│ │ Run-Rate Adjustments:                      │            │ │
│ │  (+) Full-year impact of new contract      │   +$1.8M   │ │
│ │  (+) Annualized cost savings (in progress) │   +$1.0M   │ │
│ ├────────────────────────────────────────────┼────────────┤ │
│ │ Pro Forma Run-Rate EBITDA                  │   $47.1M   │ │
│ └────────────────────────────────────────────┴────────────┘ │
│                                                             │
│ ASSESSMENT: ~5% adjustment gap — within normal range        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## Color Coding Standards

| Element | Color | Usage |
|---------|-------|-------|
| Positive/Outperform | Green (#22c55e) | Above benchmark, growth |
| Neutral/In-line | Yellow (#eab308) | At benchmark |
| Negative/Underperform | Red (#ef4444) | Below benchmark, risks |
| Highlight/Base Case | Blue (#3b82f6) | Base case, key metrics |
| Company/Focus | Navy (#1e3a8a) | Headers, company name |
| Top Quartile | Green (#81C784) | Best-in-class metrics |
| Bottom Quartile | Red (#FFCDD2) | Below-peer metrics |
| Invisible (Waterfall) | No Fill | Base series in waterfall charts |

---

## Slide Count by Deck Type

| Deck Type | Core Slides | Appendix | Total |
|-----------|-------------|----------|-------|
| Growth Equity IC | 13 | 3-5 | 16-18 |
| LBO / Buyout IC | 15-20 | 5-10 | 20-30 |
| 24-Hour LBO | 8-10 | 2-3 | 10-13 |
| Institutional Full | 25-30 | 10-15 | 35-45 |
| Quick Screening | 5-8 | 0-2 | 5-10 |

---

---

# Part 2: Equity Research Slide Templates
## From 10 Years of CFA Research Challenge Winners (2016-2025)

*14 additional templates organized by category. Each includes ASCII layout, source year, and when to use.*

### ER-1: Three-Pillar Investment Thesis (2019, 2021, 2023, 2024, 2025)
```
+-------------------------------------------------------------+
| INVESTMENT THESIS                                           |
|-------------------------------------------------------------|
|  +---------------+  +---------------+  +---------------+    |
|  |   [ICON 1]    |  |   [ICON 2]    |  |   [ICON 3]    |    |
|  |  Thesis #1    |  |  Thesis #2    |  |  Thesis #3    |    |
|  |  [Headline]   |  |  [Headline]   |  |  [Headline]   |    |
|  | * Data pt 1   |  | * Data pt 1   |  | * Data pt 1   |    |
|  | * Data pt 2   |  | * Data pt 2   |  | * Data pt 2   |    |
|  +---------------+  +---------------+  +---------------+    |
|  +-----------------------------------------------------+    |
|  |  BUY  |  Target: $XXX  |  Upside: XX%              |    |
|  +-----------------------------------------------------+    |
+-------------------------------------------------------------+
```
**When:** Opening and closing of every equity research presentation.
**Enhancement (2025):** Use company ticker as acronym (D-O-M = Dominance, Opportunity, Momentum).

### ER-2: Football Field / Valuation Summary (2018, 2019, 2021, 2023, 2024, 2025)
```
+-------------------------------------------------------------+
| VALUATION SUMMARY                                           |
|-------------------------------------------------------------|
|  52-Week Range  |-----==========-----|                      |
|  Broker Targets |------===========---|                      |
|  DCF (70%)      |----=============------|                   |
|  EV/EBITDA (30%)|---============-----|                      |
|  SOTP           |-----==========-----|                      |
|                 $XXX  ^Current  ^Target  $XXX               |
|  Blended Target: $XXX (XX% upside)  | Weights shown        |
+-------------------------------------------------------------+
```
**When:** ALWAYS include. The single most expected slide in equity research.

### ER-3: Monte Carlo Histogram (2017, 2018, 2021, 2022, 2024)
```
+-------------------------------------------------------------+
| MONTE CARLO SIMULATION (N = XXX,XXX iterations)             |
|-------------------------------------------------------------|
|  Freq ^            +--+                                     |
|       |         +--+  +--+                                  |
|       |      +--+  |  |  +--+                               |
|       |   +--+  |  |  |  |  +--+                            |
|       |+--+  |  |  |  |  |  |  +--+                         |
|       +---+--+--+--+--+--+--+--+--+--> Price                |
|        RED     YELLOW    GREEN                               |
|        ^Current         ^Target                              |
|  Mean: $XXX | Median: $XXX | Std Dev: $XX                   |
|  P(>10% upside): XX% | P(SELL): XX%                        |
+-------------------------------------------------------------+
```
**When:** 80% of CFA winners use this. Highest-priority gap for IB toolkit.

### ER-4: SOTP Waterfall (2016, 2019, 2025)
```
+-------------------------------------------------------------+
| SUM-OF-THE-PARTS VALUATION                                  |
|-------------------------------------------------------------|
|  $XXX +                                                     |
|       |  +---+                                              |
|       |  | A |  +---+                                       |
|       |  |   |  | B |  +---+                                |
|       |  |   |  |   |  | C |  +---+  +---+  ===            |
|       |  |   |  |   |  |   |  |+Ca|  |-De|  |EV|           |
|       +--+---+--+---+--+---+--+---+--+---+--+==+           |
|       Seg A   Seg B   Seg C   Cash   Debt   Equity          |
|       (DCF)   (Comps) (P/FFO) (+)    (-)    = $XX/sh       |
+-------------------------------------------------------------+
```

### ER-5: DuPont Decomposition (2018, 2019, 2020, 2023, 2024, 2025)
```
+-------------------------------------------------------------+
| ROE DECOMPOSITION (DuPont)                                  |
|-------------------------------------------------------------|
|  +------------+  +------------+  +------------+  +------+   |
|  | Net Profit  |  | Asset      |  | Equity     |  |      |   |
|  | Margin      | x| Turnover   | x| Multiplier | =| ROE  |   |
|  |   17% dn   |  |   0.6x ->  |  |   3.1x dn |  | 32%  |   |
|  +------+-----+  +------+-----+  +------+-----+  +------+   |
|         |               |               |                     |
|    [Declining:     [Stable:        [Declining:                |
|     cost pressure]  asset-light]    deleveraging]             |
+-------------------------------------------------------------+
```
**When:** 70% frequency. High value for peer comparison and trend analysis.

### ER-6: Risk Matrix (2017, 2019, 2021, 2022, 2023, 2024, 2025)
```
+-------------------------------------------------------------+
| RISK ASSESSMENT                                             |
|-------------------------------------------------------------|
|  IMPACT ^                                                    |
|  HIGH  |         [R3]*          [R1]#                       |
|  MED   |  [R5]+        [R2]*          [R4]#                |
|  LOW   |         [R6]+                                      |
|        +--------------------------------------> PROBABILITY  |
|           LOW        MED         HIGH                       |
|  * Market Risk  # Operational  + Political/Regulatory       |
|  R1: [Name] -- Valuation impact: -XX%                       |
|      Mitigant: [Specific action/data]                       |
+-------------------------------------------------------------+
```
**When:** 100% of CFA winners use this. ALWAYS quantify valuation impact per risk.

### ER-7: Scenario Analysis Three-Column (2021 Vestas)
```
+-------------------------------------------------------------+
| SCENARIO ANALYSIS                                           |
|-------------------------------------------------------------|
|  +----------+    +--------------+    +----------+           |
|  | BEAR     |    |  BASE        |    | BULL     |           |
|  | $XXX     |    |  $XXX        |    | $XXX     |           |
|  | -XX%     |    |  +XX%        |    | +XX%     |           |
|  +----------+    +--------------+    +----------+           |
|  |Thesis 1: |    |Thesis 1:     |    |Thesis 1: |           |
|  | [Bear]   |    | [Base]       |    | [Bull]   |           |
|  |Thesis 2: |    |Thesis 2:     |    |Thesis 2: |           |
|  | [Bear]   |    | [Base]       |    | [Bull]   |           |
|  +----------+    +--------------+    +----------+           |
+-------------------------------------------------------------+
```
**When:** Link each scenario to investment thesis pillars, not just numbers.

### ER-8: Porter's Five Forces Pentagon (2017 Copa)
```
+-------------------------------------------------------------+
| PORTER'S FIVE FORCES                                        |
|-------------------------------------------------------------|
|                    New Entrants                              |
|                    ***oo (3/5)                               |
|                       / \                                    |
|  Supplier Power  ***oo    **ooo  Buyer Power                |
|     (3/5)          /        \     (2/5)                     |
|              ****o          ***oo                            |
|          Rivalry (4/5)    Substitutes (3/5)                  |
|  Overall: MODERATE competitive intensity                    |
+-------------------------------------------------------------+
```
**When:** Quantify each force with data, don't just say "moderate."

### ER-9: Competitive Advantage Flywheel (2021 Vestas)
```
+-------------------------------------------------------------+
| COMPETITIVE ADVANTAGE CYCLE                                 |
|-------------------------------------------------------------|
|           +------------------+                              |
|           |  Higher R&D      |                              |
|           |  Spending        |                              |
|           +--------+---------+                              |
|                    v                                         |
|  +-----------+         +----------------+                   |
|  | Higher    |    <    | Superior       |                   |
|  | Profitab. |         | Technology     |                   |
|  +-----+-----+         +--------+-------+                   |
|        ^                        v                            |
|        +----------Premium Pricing+                          |
|  Result: 2x peer efficiency, 11% higher ASP                |
+-------------------------------------------------------------+
```
**When:** Companies with technology moats or self-reinforcing cycles.

### ER-10: Value Chain Flow (2019, 2024)
```
+-------------------------------------------------------------+
| VALUE CHAIN POSITIONING                                     |
|-------------------------------------------------------------|
|  [Mfg] -> [First Mile] -> [MIDDLE MILE] -> [Last Mile] -> [C]|
|                          || COMPANY    ||                    |
|                          || 90% share  ||                    |
|                          || $XXM rev   ||                    |
|  Key: Company controls the critical segment                 |
|  Barriers: $413M insourcing cost, regulatory protection     |
+-------------------------------------------------------------+
```

### ER-11: ESG Scorecard Comparison (2021, 2024, 2025)
```
+-------------------------------------------------------------+
| ESG ASSESSMENT                                              |
|-------------------------------------------------------------|
|  Dimension    Company  Peer Avg   Rating                    |
|  Environment   3.5      2.8       ========oo Above          |
|  Social        4.0      3.2       =========o Above          |
|  Governance    3.8      2.9       ========oo Above          |
|  OVERALL       3.8      3.0       ========oo Above          |
|  External: MSCI: A | Sustainalytics: 22.6 | Bloomberg: 3.8 |
+-------------------------------------------------------------+
```
**When:** Post-2020, every winning report includes ESG. Compare proprietary vs external.

### ER-12: "Did You Know" Hook Slide (2025 Kozminski)
```
+-------------------------------------------------------------+
|                                                             |
|            "Average Per Sqm Prices Have                     |
|             DOUBLED in 6 Years"                             |
|        PLN 7,328                PLN 14,867                  |
|        (2018)                   (2024)                      |
|                                                             |
+-------------------------------------------------------------+
```
**When:** Slide 2 or 3. One striking statistic that hooks the audience.

### ER-13: KPI Dashboard (2025 Kozminski)
```
+-------------------------------------------------------------+
| COMPANY SNAPSHOT                                            |
|-------------------------------------------------------------|
|  +--------+ +--------+ +--------+ +--------+ +--------+    |
|  |  53K   | |  18yr  | |  ~5yr  | |  32%   | |  17%   |    |
|  | Units  | | Divid. | | Land   | | Gross  | | Net    |    |
|  | Sold   | | Track  | | Bank   | | Margin | | Margin |    |
|  +--------+ +--------+ +--------+ +--------+ +--------+    |
|  +--- Revenue ---+  +-- Peers --+  +-- Geography --+       |
|  | [Bar chart]   |  | [H-bars]  |  | [Donut chart] |       |
|  | 5Y with CAGR  |  | by units  |  | by city       |       |
|  +---------------+  +-----------+  +---------------+       |
+-------------------------------------------------------------+
```

### ER-14: Peer Comparison Quad Chart (2018 Lausanne)
```
+-------------------------------------------------------------+
| PEER BENCHMARKING                                           |
|-------------------------------------------------------------|
|  +------------------------+  +------------------------+     |
|  | EBITDA Margin (%)      |  | ROIC (%)               |     |
|  | Co: ======== 55%       |  | Co: ======== 15%       |     |
|  | P1: ===== 35%          |  | P1: ==== 8%            |     |
|  | P2: ==== 30%           |  | P2: === 6%             |     |
|  +------------------------+  +------------------------+     |
|  +------------------------+  +------------------------+     |
|  | FCF Margin (%)         |  | Operating Cycle (days) |     |
|  | Co: ======= 20%        |  | Co: ==== 112           |     |
|  | P1: ==== 12%           |  | P1: ======== 187       |     |
|  +------------------------+  +------------------------+     |
+-------------------------------------------------------------+
```

---

## Combined Slide Count Benchmarks

| Deck Type | Core Slides | Appendix | Total |
|-----------|-------------|----------|-------|
| Growth Equity IC | 13 | 3-5 | 16-18 |
| LBO / Buyout IC | 15-20 | 5-10 | 20-30 |
| 24-Hour LBO | 8-10 | 2-3 | 10-13 |
| Institutional Full | 25-30 | 10-15 | 35-45 |
| Quick Screening | 5-8 | 0-2 | 5-10 |
| **Equity Research (CFA)** | **22-52** | **100-186** | **130-238** |

**CFA Key Insight:** The appendix IS the differentiator. 150+ appendix slides signals "ask me anything" depth.

---

## Template Index

### Part 1: IC Presentation Templates (25+ slides)
- GE Slide 1-13: Title, Exec Summary, Company, Thesis, Market, Competitive, Unit Economics, Financials, Returns (x2), Risk, What Must Be True, Recommendation
- LBO Additional: Sources & Uses, Value Creation Bridge, Football Field, SaaS Benchmarking, Precedent Transactions, DCF, Sensitivity, IRR Attribution, Debt Schedule, Mgmt vs Buyer, Operating Model, Add-on/Synergy, Cap Table, 100-Day Plan, Covenant, QoE

### Part 2: Equity Research Templates (14 slides)
- ER-1: Three-Pillar Investment Thesis
- ER-2: Football Field / Valuation Summary
- ER-3: Monte Carlo Histogram
- ER-4: SOTP Waterfall
- ER-5: DuPont Decomposition
- ER-6: Risk Matrix
- ER-7: Scenario Analysis Three-Column
- ER-8: Porter's Five Forces Pentagon
- ER-9: Competitive Advantage Flywheel
- ER-10: Value Chain Flow
- ER-11: ESG Scorecard Comparison
- ER-12: "Did You Know" Hook Slide
- ER-13: KPI Dashboard
- ER-14: Peer Comparison Quad Chart

**Total: 39+ unique slide templates**

---

*Version: 3.0 | Last Updated: February 2026*
*Part 1: BCI 2026 Case Study, Northleaf 2026 Model, and institutional presentations*
*Part 2: 10 years of CFA Research Challenge Global Winners (2016-2025)*
*For design specs and python-pptx code, see `SLIDE_DESIGN_GUIDE.md`*
