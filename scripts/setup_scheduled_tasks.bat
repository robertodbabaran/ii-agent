...
schtasks /create /tn "II-Agent Net Worth Newsletter" /tr "%~dp0..\src\ii_skills\networth_newsletter\run_networth.bat" /sc daily /st 07:00 /ru "%USERNAME%" /rl HIGHEST

REM Create Market Newsletter task (7:15 AM daily)
echo Creating Market Newsletter task (7:15 AM)...
schtasks /create /tn "II-Agent Market Newsletter" /tr "%~dp0..\src\ii_skills\market_newsletter\run_newsletter.bat" /sc daily /st 07:15 /ru "%USERNAME%" /rl HIGHEST

REM Create WHOOP Dashboard task (7:30 AM daily)
echo Creating WHOOP Dashboard task (7:30 AM)...
schtasks /create /tn "II-Agent WHOOP Dashboard" /tr "%~dp0..\src\ii_skills\health_dashboard\run_whoop.bat" /sc daily /st 07:30 /ru "%USERNAME%" /rl HIGHEST
